...
def build_boleta_detalle(*, personal, boleta, anio, mes, dias_con_marcacion, dias_justificados, dias_descanso, faltas):
    ...
    ingresos, descuentos, aportes_trabajador, aportes_empleador = [], [], [], []
    ...
    return {
        ...
        "documento_identidad": {
            "tipo": (personal.tipo_documento or "").strip() or "Documento",
...
def _assignment_for_day(asignaciones_turno, current_day):
    fallback = None
...
def build_reporte_general_payload(personal, anio, mes):
    ...
    bloques_by_asignacion = {
        item.id: sorted(list(item.turno.bloques.all()), key=lambda b: b.orden)
        for item in asignaciones_turno
    }
    ...
    for offset in range(ultimo_dia):
        ...
        entradas = [m for m in marks if m.tipo_evento == Marcacion.TipoEvento.ENTRADA]
        salidas = [m for m in marks if m.tipo_evento == Marcacion.TipoEvento.SALIDA]

        active_assignment = _assignment_for_day(asignaciones_turno, current)
        ...
        if estado_dia == ReporteAsistenciaDiaria.EstadoDia.FALTA:
        ...
        for block_index, block in enumerate(blocks_to_render, start=1):
            ...
            dias.append({
                ...
                "observacion": "",
                "marcaciones": [
                    {"id": m.id, "fecha_hora": m.fecha_hora.isoformat(), "tipo_evento": m.tipo_evento}
                    for m in marks
                ],
            })
    ...
    total_dias_laborados = sum(
        1 for d in dias if d["estado_dia"] == ReporteAsistenciaDiaria.EstadoDia.ASISTIO
    )
